...
| `/selectAllGoods`    | `List<Product>` | `无`               | `获取所有商品信息`                     |
| `/selectGoodsByType` | `List<Product>` | `ptype_id(类型id)` | `通过类型获取对应的商品信息`           |
| `/selectGoodsByKeys` | `List<Product>` | `String key`       | `根据搜索查商品`                       |
| `/insertWR.do`       | `String` "ok"   | `Product product`  | `添加商品`                             |
| `/soldOudGoods`      | `String` "ok"   | `pro_id`           | `下架或上架商品，在业务层判断`         |
| `/deleteGoods`       | `String "ok"`   | `pro_id`           | `彻底删除该商品，删除前要给与确认`     |
| `/`                  |                 |                    | `出售商品有点没搞清楚逻辑先缓缓`       |
